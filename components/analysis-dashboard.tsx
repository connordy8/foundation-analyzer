  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        <StatCard icon={<DollarSign className="h-4 w-4" />} label="Total Revenue" value={formatCurrency(filing.totrevenue || 0)} />
        <StatCard icon={<TrendingUp className="h-4 w-4" />} label="Total Expenses" value={formatCurrency(filing.totfuncexpns || 0)} />
        <StatCard icon={<Building2 className="h-4 w-4" />} label="Total Assets" value={formatCurrency(filing.totassetsend || 0)} />
        <StatCard
      ...
      <div className="flex items-center gap-2">
        ...
        <span className="text-sm font-medium text-ma-gray-700">Geographic Focus:</span>
        <span
      ...
      {!hasGrantData && (
        <div className="bg-ma-yellow/10 border border-ma-yellow/30 rounded-lg p-4 text-sm text-ma-gray-700">
          ...
          <a href={filing.pdf_url} target="_blank" rel="noopener noreferrer" className="text-ma-teal underline">
            PDF filing
          </a>{" "}
