...
export function AnalysisDashboard({ result, onBack }: AnalysisDashboardProps) {
  const { organization, filing, fitScore, causeAreaBreakdown, topRecipients, geographicFocus, hasGrantData, taxYear } = result;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-start gap-4">
        <button
          onClick={onBack}
          className="mt-1 p-2 rounded-lg hover:bg-ma-gray-100 transition-colors text-ma-gray-500"
        >
          <ArrowLeft className="h-5 w-5" />
        </button>
        <div className="flex-1">
          <h1 className="text-2xl font-bold text-ma-gray-900">
            {organization.name}
          </h1>
          <div className="flex flex-wrap items-center gap-3 mt-1 text-sm text-ma-gray-500">
            ...
            <span className="px-2 py-0.5 bg-ma-teal/10 text-ma-teal rounded text-xs font-medium">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        <StatCard
          ...
          label="Total Revenue"
          value={formatCurrency(filing.totrevenue || 0)}
        />
        <StatCard
          icon={<TrendingUp className="h-4 w-4" />}
          label="Total Expenses"
          value={formatCurrency(filing.totfuncexpns || 0)}
        />
        <StatCard
          icon={<Building2 className="h-4 w-4" />}
          label="Total Assets"
          value={formatCurrency(filing.totassetsend || 0)}
        />
        <StatCard
      ...
      <div className="flex items-center gap-2">
        {geographicFocus.type === "National" ? (
          ...
          <MapPin className="h-4 w-4 text-ma-orange" />
        )}
        <span className="text-sm font-medium text-ma-gray-700">
          Geographic Focus:
        </span>
        <span
          className={`px-3 py-1 rounded-full text-sm font-medium ${
            geographicFocus.type === "National"
              ...
              : "bg-ma-orange/10 text-ma-orange"
          }`}
        >
      ...
      {!hasGrantData && (
        <div className="bg-ma-yellow/10 border border-ma-yellow/30 rounded-lg p-4 text-sm text-ma-gray-700">
          <strong>Note:</strong> No XML e-file data was available for this
          organization. The fit score is based on limited data. Check the{" "}
          <a
            href={filing.pdf_url}
            target="_blank"
            rel="noopener noreferrer"
            className="text-ma-teal underline"
          >
            PDF filing
          </a>{" "}
      ...
      <div className="grid md:grid-cols-2 gap-6">
        <div className="bg-white rounded-xl border border-ma-gray-200 p-6 flex items-center justify-center">
...
function StatCard({
  ...
  return (
    <div className="bg-white rounded-lg border border-ma-gray-200 p-4">
      ...
      <p className="text-xl font-bold text-ma-gray-900">{value}</p>
      {subtitle && (
        <p className="text-xs text-ma-gray-500 mt-0.5">{subtitle}</p>
      )}
    </div>
  );
}
